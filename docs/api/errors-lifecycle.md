...
Wallet close detaches, flushes and invalidates its resources; injected clients and signers remain caller-owned. On worker failure, invalidate the affected domain and reopen durable state; never replay a mutation automatically. Finalizers are leak backstops, not lifecycle correctness.
