...
try {
  ...
  for (const [name, expected] of Object.entries(manifest.files)) {
    if (!/^[a-zA-Z0-9_.-]+$/.test(name)) throw Error('invalid asset name');
  ...
  if (results.filter(r => r.category === 'runtime').length !== 14 || results.filter(r => r.category === 'harness-control').length !== 5) throw Error('scenario count mismatch');
  ...
  await writeFile(resultPath, JSON.stringify({ id, exitCode, capabilities, manifest, results, events, records,
    limits: 'Ephemeral memdb only; no full F1/#2, scanner, storage durability or threading completion.' }, null, 2) + '\n');
  process.exitCode = exitCode;
}
