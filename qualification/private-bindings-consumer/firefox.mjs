...
try {
  const sums = await readFile(join(root, 'SHA256SUMS'));
  assert.match(process.argv[2] || '', /^[0-9a-f]{64}$/, 'provide pinned SHA256SUMS digest');
  assert.equal(sha256(sums), process.argv[2], 'package hash mismatch');
  report.packageSha256 = process.argv[2];
  const assets = new Map();
  for (const line of sums.toString().trim().split('\n')) {
    assert.match(line, /^[0-9a-f]{64}  [a-zA-Z0-9_.-]+$/);
    const name = line.slice(66);
    assert.ok(!assets.has('/' + name), 'duplicate package member');
    const bytes = await readFile(join(root, name));
    assert.equal(sha256(bytes), line.slice(0, 64), `package member ${name}`);
    assets.set('/' + name, bytes);
  }
  assert.equal(sha256(await readFile(fileURLToPath(import.meta.url))), sha256(assets.get('/firefox.mjs')), 'runner bytes');
  report.pin = JSON.parse(assets.get('/pin.json'));
  assets.set('/', Buffer.from(`<!doctype html><title>Private binding consumption qualification</title>
<script type="module">
  window.privateBindingsResult = import('/browser.mjs').then(m => m.run()).then(
