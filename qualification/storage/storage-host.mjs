...
function attempt(op, file, fallback, fn) {
  try {
    ...
    record(op, file, { rc });
    const c = state.crash;
    if (c && c.op === op && (!c.file || c.file === file) && --c.nth === 0) {
      // The parent observes this point after the real primitive, then kills us.
      backend.crash({ checkpoint: { op, file }, trace: state.trace });
      for (;;) { /* external worker deadline/termination, no shared memory */ }
    }
    return rc;
  } catch (e) {
...
export function entropy(ptr, n) {
  ...
  crypto.getRandomValues(bytes(ptr, n)); return n;
}
