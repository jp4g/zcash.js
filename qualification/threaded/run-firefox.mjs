...
const deadline = setTimeout(() => stop.abort(Error('Firefox suite deadline exceeded')), 180000);
...
stop.signal.addEventListener('abort', () => rejectPending(stop.signal.reason), { once: true });
let browserIdentity;
...
async function request(route, method = 'GET', body, { timeout = 60000, cleanup = false } = {}) {
...
try {
  ...
  for (const [name, expected] of Object.entries(manifest.files)) {
    if (!/^[a-zA-Z0-9_./-]+$/.test(name) || name.split('/').includes('..')) throw Error('invalid asset name');
  ...
  server = createServer((req, res) => {
    const isolated = req.url.startsWith('/isolated/');
    const relative = req.url.replace(/^\/(isolated|baseline)\//, '/');
  ...
  const driverArgs = ['--host', '127.0.0.1', '--port', '19447', '--websocket-port', '0', '--profile-root', runRoot];
  record({ stage: 'launch', geckodriver, driverArgs, firefox, origin, sandbox: 'unchanged', coop: 'same-origin on /isolated/', coep: 'require-corp on /isolated/' });
  ...
  await request(`/session/${session}/timeouts`, 'POST', { script: 55000, pageLoad: 15000, implicit: 0 });
  ...
  for (const scenario of ['no-sab', 'shared', 'owner-error', 'owner-stall', 'compute-error', 'compute-stall']) {
    const prefix = scenario === 'no-sab' ? 'baseline' : 'isolated';
    const page = `${origin}/${prefix}/`;
    await request(`/session/${session}/url`, 'POST', { url: page });
    ...
    if (realms.length !== 1 || realms[0].origin !== origin) throw Error('page realm missing');
    const owner = realms[0].realm, after = events.length;
    record({ stage: 'scenario-start', scenario, after, owner, page });
    const result = await execute("const scenario=arguments[0],done=arguments[arguments.length-1]; import('./browser-controller.mjs').then(m=>m.runScenario(scenario)).then(result=>done({success:true,result}),e=>done({success:false,error:String(e),stack:e.stack}));", [scenario]);
    const wanted = scenario === 'no-sab' ? 1 : scenario === 'shared' ? 3 : scenario.startsWith('owner-') ? 2 : 4;
    const workerURLs = ['browser-worker.mjs','browser-baseline.mjs'].map(name => page + name);
    const lifecycle = await waitFor(() => requireLifecycle(events, { after, wanted, owner, origin, workerURLs }), 8000, `worker lifecycle ${scenario}`);
    const live = await bidi('script.getRealms', { type: 'dedicated-worker' });
    if (!Array.isArray(live.realms) || live.realms.length) throw Error(`live worker leak: ${JSON.stringify(live)}`);
    requireLifecycle(events, { after, wanted, owner, origin, workerURLs });
    if (!result.ok || result.scenario !== scenario) throw Error('scenario failed');
    results.push({ ...result, lifecycle }); record({ stage: 'scenario-pass', ...result, lifecycle });
  }
  record({ stage: 'complete', scenarios: results.length });
  exitCode = 0;
} catch (error) {
  ...
  await writeFile(resultPath, JSON.stringify({ id, exitCode, capabilities, manifest, results, events, records,
    limits: 'Synthetic threaded runtime only. Ephemeral SQLite; scanner parity, full wallet/prover F1 and F2 remain open.' }, null, 2) + '\n');
  process.exitCode = exitCode;
}
