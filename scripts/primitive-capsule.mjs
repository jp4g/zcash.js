...
const profiles=[
  {name:'primitive',receipt:'aa8b4d24eb510cf176d4114372691df27637f1dd7fbbdbccddd46e72cbf2184f',directory:'primitive',receiptFile:'build.json',artifacts:'primitiveArtifacts',wasm:'bindings_bg.wasm',modules:['bindings.js','bytes.mjs','network.mjs','transaction.mjs']},
...
let codec;
export function initialize(){return codec??=${profile.factory}(Uint8Array.from(atob('${wasm.toString('base64')}'),c=>c.charCodeAt(0)));}`:`import {initialize as initializeNative,consensusContext,openViewingAuthority,decodeViewingAddress,selectViewingReceiver,validateBirthday,parseStandalonePczt} from '${join(source,'network.mjs')}';
...
export {consensusContext,decodeTransaction,openViewingAuthority,decodeViewingAddress,selectViewingReceiver,validateBirthday,parseStandalonePczt};`;
