...
export async function resolveBirthday(
  args: { light: LightClient; firstScanHeight: number; recoverUntilExclusive?: number } & Op,
): Promise<Birthday> {
  ...
  if (!Number.isInteger(first) || first < 1 || first > 0xffff_ffff
    || recover !== undefined
    && (!Number.isInteger(recover) || recover < first || recover > 0xffff_ffff)) throw invalidArgument();
  ...
  try {
    pending = operation(owned.signal);
  } catch {
  ...
  try {
    ...
    const result = snapshot(
      await pending.wait(Reflect.apply(read, light, [{ height: first - 1, signal: pending.signal }])),
    ...
    const dto = initialize().decodeResponse('GetTreeState', bytes) as {
      height: string;
      hash: string;
      sapling_tree: string;
      ironwood_tree: string;
    };
    if (dto.height !== String(point.height) || dto.hash !== point.hash) throw invalidBirthday();
    for (const [value, encoded] of [
      [result.sapling, dto.sapling_tree],
      [result.ironwood, dto.ironwood_tree],
    ] as const) {
      if (value === null) {
        if (encoded !== '') throw invalidBirthday();
      } else {
        const tree = ownBytes(value, invalidBirthday, resource);
        if (encoded.length !== tree.length * 2
          || tree.some(
            (byte, i) => byte.toString(16).padStart(2, '0') !== encoded.slice(i * 2, i * 2 + 2),
    ...
    return {
      network,
      firstScanHeight: first,
      priorTreeState: bytes,
      ...(recover === undefined ? {} : { recoverUntilExclusive: recover }),
      source: 'light-client',
    };
  } catch (error) {
    pending.check();
