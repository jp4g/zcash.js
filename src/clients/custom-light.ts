...
const unaryMethods = new Set<LightUnaryMethod>(
  [
    'GetLatestBlock',
    'GetLightdInfo',
    'GetTransaction',
...
const normalized = (error: unknown) => isZcashError(error)
  ? error
  : failure(
      'TRANSPORT_ERROR',
      'transport',
      'configure',
      'Custom light request failed.',
    );
...
export function ownCustomLightTransport(value: CustomLightTransport): CustomLightTransport {
  ...
  try {
    ...
    for (const key of Reflect.ownKeys(value)) {
      if (typeof key !== 'string'
        || !['kind', 'sourceId', 'protocolRevision', 'unary', 'stream'].includes(key)) throw 0;
  ...
  admit(captured, {}, []);
  function request<T extends LightUnaryMethod | LightStreamMethod>(
    args: { method: T; request: Uint8Array } & Op,
    methods: Set<T>,
  ) {
  ...
  return Object.freeze({
    ...
    protocolRevision: captured.protocolRevision,
    async unary(args: Parameters<CustomLightTransport['unary']>[0]) {
      const input = request(args, unaryMethods),
      ...
      try {
        ...
        const result = await pending.wait(
          Reflect.apply(
            captured.unary,
            value,
            [{ method: input.method, request: input.request, signal: pending.signal }],
          ),
        );
        pending.check();
    ...
    stream(args: Parameters<CustomLightTransport['stream']>[0]): AsyncIterableIterator<Uint8Array> {
      ...
      function release() {
        ...
        released = owned;
        try {
          void Promise.resolve(owned.return?.()).catch(() => { });
        } catch { /* Foreign cleanup cannot delay or replace terminality. */ }
      }
      return {
        ...
        async next() {
          ...
          try {
            if (!pending) {
              pending = operation(input.signal, release);
              pending.check();
