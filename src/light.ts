...
export function createLightClient(
  ...
  try {
    endpoint = new URL(grpcBinding(input.transport as GrpcTransport).url).href;
    selected = grpcAdapter(input.transport as GrpcTransport);
  } catch {
    selected = input.transport as CustomLightTransport;
  }
  ...
  async function ready(signal: AbortSignal) {
    ...
    try {
      ...
      throw failure('RUNTIME_UNAVAILABLE', 'runtime', 'configure', 'Light codecs unavailable.');
    }
    signal.throwIfAborted();
    if (!verified) {
      const info = await readLightdInfo({ codec: value.wire, transport, sourceId: transport.sourceId }, { signal });
      const branch = codec.consensusContext(
        definition.parametersFormat,
        definition.parameters.bytes,
        info.blockHeight,
      ).branchId;
      ...
      for await (const block of chain.streamCompactBlocks(
        value.wire,
        transport,
        { fromHeight: 1, toHeight: 1, signal },
      )) {
  ...
  function source(value: Native): transactions.LightTransactionSource {
    return {
      ...
      validateAddress(address) {
        try {
          return value.address.decode(address, family).canonical;
        } catch {
      ...
      async decodeTransaction(raw, height) {
        ...
        const branches = new Set(
          heights.map(
            at => codec.consensusContext(definition.parametersFormat, definition.parameters.bytes, at).branchId,
          ),
        );
        for (const branch of branches) {
          try {
            return codec.decodeTransaction(raw, branch);
          } catch { /* Try the next registered native context. */ }
        }
        throw invalidArgument();
      },
    };
  ...
  async function unary<A extends Op, T>(
    ...
    call: (value: Native, args: A) => Promise<T>,
    broadcast = false,
  ): Promise<T> {
  ...
  function stream<A extends Op, T>(
    args: A,
    keys: readonly string[],
    call: (value: Native, args: A) => AsyncIterable<T>,
    ...
    return {
      ...
      async next() {
        ...
        busy = true;
        try {
          pending ??= chain.operation(owned.signal, () => {
  ...
  const client = Object.freeze({
    ...
    streamCompactBlocks: args => stream(
      args,
      ['fromHeight', 'toHeight'],
      (v, a) => chain.streamCompactBlocks(v.wire, transport, a),
    ),
    getTransaction: args => unary(args, ['txid'], (v, a) => transactions.getTransaction(source(v), a)),
    getSubtreeRoots: args => stream(
      args,
      ['pool', 'startIndex', 'limit'],
      (v, a) => transactions.getSubtreeRoots(source(v), a),
    ),
    streamAddressTransactions: args => stream(
      args,
      ['address', 'fromHeight', 'toHeight'],
