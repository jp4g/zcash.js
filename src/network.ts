...
const networks = new WeakMap<
  Network,
  Readonly<{ definition: ReturnType<typeof bindNetworkDefinition>; codec: Codec }>
>();
...
export async function defineNetwork(args: NetworkDefinition & Op): Promise<Network> {
  ...
  try {
    ...
    for (const key of Reflect.ownKeys(args)) {
      if (typeof key !== 'string'
        || !['identity', 'genesisHash', 'parameters', 'parametersFormat', 'signal'].includes(key)) throw 0;
  ...
  const definition = bindNetworkDefinition({
    identity: input.identity,
    genesisHash: input.genesisHash as NetworkDefinition['genesisHash'],
    parametersFormat: input.parametersFormat as string,
    parameters,
  });
  ...
  try {
    ...
    throw failure('RUNTIME_UNAVAILABLE', 'runtime', 'configure', 'Network codecs unavailable.');
  }
  ...
  try {
    native.consensusContext(definition.parametersFormat, parameters, 0);
  } catch {
    throw invalidArgument();
  }
