...
export function consensusContext(
  format: string,
  parameters: Uint8Array,
  height: number,
): Readonly<{ height: number; branchId: number }>;
export function decodeTransaction(bytes: Uint8Array, branch: number): Readonly<
...
export interface ViewingAuthority {
  describe(): {
    kind: 'ufvk' | 'uivk';
    components: import('../types.js').ReceiverType[];
    enabledPools: import('../types.js').Pool[];
    provenance: null;
  };
  ...
  find(
    ...
    request: string,
    maxAttempts: number
  ): Omit<import('../types.js').AddressRecord, 'index'> & { index: string };
  dispose(): void;
}
export function openViewingAuthority(
  parameters: Uint8Array,
  format: string,
  encoded: string,
  pools: string,
): ViewingAuthority;
export function decodeViewingAddress(
  parameters: Uint8Array,
  encoded: string,
): Omit<import('../types.js').DecodedAddress, 'network'>;
export function selectViewingReceiver(
  parameters: Uint8Array,
  encoded: string,
  pool: string,
  height: number,
  branch: number,
): import('../types.js').SelectedReceiver;
...
export function parseStandalonePczt(
  parameters: Uint8Array,
  genesis: Uint8Array,
  height: number,
  branch: number,
