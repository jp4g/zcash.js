...
// No threaded executable is admitted until the actual package closure is reviewed.
...
async function createOwner(baseline: WasmArtifact, runtime: Record<string, any>, provingCapacity: number, signal: AbortSignal, forget: () => void, threaded = false) {
  ...
  try {
    ...
    try {
      ...
      }) || !sameRecord(identity.memory, { initialPages: 321, maximumPages: 4096, shared: threaded })) throw mismatch();
      poolReady = true;
