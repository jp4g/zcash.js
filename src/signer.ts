...
export function signerCapabilities(value: unknown): SignerCapabilities {
  ...
  return { revision:string(owned.revision), networks:list(owned.networks,string),
    ...
    exportableViewing:list(owned.exportableViewing, format => member(format,['ufvk','uivk'])), maxPcztBytes:owned.maxPcztBytes };
}
function response<T>(read: () => T): T {
  try { return read(); } catch (error) { if (isZcashError(error) && error.code === 'RESOURCE_LIMIT') throw error; throw protocol(); }
}
  ...
  try {
    ...
    for (const name of captured?[]:['getCapabilities','getAccount','authorize'] as const) {
      ...
      while (owner && !property) {
        ...
        seen.add(owner); property = Object.getOwnPropertyDescriptor(owner, name);
