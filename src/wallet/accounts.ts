...
export function walletAccounts(wallet: Wallet, network: Network) {
  ...
  const check = () => {
    if (closing) throw failure('CLOSED', 'account', 'none', 'Wallet accounts are closed.');
    wallet.session.check();
  };
  ...
  const project = (record: AccountRecord): AccountRecord => ({
    ...record,
    signerAttached: record.signerAttached
      || Boolean(
        bindings.has(record.id) && !memorySignerAuthority(bindings.get(record.id)!.signer),
      ),
  });
  async function mnemonic(
    ...
    try {
      ...
      try {
        pending.check();
        check();
        return { account: created.account, signer };
      } catch (error) {
        await signer.dispose().catch(() => { });
        throw error;
      }
    } catch (error) {
  ...
  const api = Object.freeze({
    ...
    async get(args) {
      check();
      const record = await wallet.session.accounts.get(snapshot(args, ['accountId', 'signal']));
      return record ? project(record) : null;
    },
    async remove(args) {
      check();
      const input = snapshot(args, ['accountId', 'acknowledge', 'signal']),
    ...
    async attachSigner(args) {
      check();
      const input = snapshot(args, ['accountId', 'signer', 'signal']),
        id = accountId(input.accountId);
      if (attaching.has(id)) throw failure('STORAGE_BUSY', 'account', 'none', 'Signer attachment is pending.');
      const pending = operation(input.signal);
      ...
      try {
        ...
        if (authority) {
          ...
          const descriptor = await signer.getAccount({
            network,
            selector: await signerSelector(wallet.session, account, pending.signal),
        ...
        const binding: SignerBinding = Object.freeze({
          accountId: input.accountId,
          state,
          dispose() {
            return disposed ??= Promise.resolve().then(async () => {
              if (bindings.get(id)?.binding !== binding) return;
              try {
                await unbind?.();
              } catch (error) {
                if (!error || typeof error !== 'object'
                  || !['CLOSED', 'STALE_HANDLE', 'WORKER_CRASHED'].includes((error as { code: string }).code)) {
                  throw error;
                }
              }
              bindings.delete(id);
        ...
        bindings.set(id, { binding, signer: input.signer });
        published = true;
        return binding;
      } catch (error) {
        if (closing) check();
        ...
        attaching.delete(id);
        pending.close();
        if (!published) await unbind?.().catch(() => { });
      }
    },
    async detachSigner(args) {
      check();
      const input = snapshot(args, ['accountId', 'signal']),
        id = accountId(input.accountId),
  ...
  return Object.freeze({
    api,
    attachedSigner(id: string) {
      check();
      return bindings.get(id)?.signer;
    },
    close() {
      if (closing) return closing;
      closing = Promise.resolve().then(async () => {
        try {
          for (const { binding } of bindings.values()) await binding.dispose();
        } finally {
          bindings.clear();
          await wallet.close();
        }
      });
      for (const pending of attaching.values()) pending.cancel();
      return closing;
    },
  });
}
