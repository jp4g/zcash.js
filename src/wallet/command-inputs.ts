...
const provingBounds: CopyBounds = {fields: {
  spend: {bytes: saplingAssets[0].byteLength}, output: {bytes: saplingAssets[1].byteLength},
}};
const inventoryBounds: CopyBounds = {fields: {result: {fields: {
...
function accountInput(command: WalletCommand, args: unknown) {
  ...
  void _network;
  return {...input, birthday: {...checkpoint, parameters: definition.parameters.bytes,
...
export function prepareCommand(command: WalletCommand, args: object, pcztMaximum: number): {
  ...
  switch (command) {
    ...
    case 'pczt_prove':
    case 'pczt_finalize':
      return result({...fields(args as unknown, ['operationId', 'artifactId', 'spend', 'output', 'signal']), maximum}, provingBounds);
    case 'fused_send':
      return result({...fields(args as unknown, ['operationId', 'proposalId', 'reviewCommitment', 'token', 'spend', 'output', 'signal']), maximum}, provingBounds);
    case 'pczt_import':
      return result({...fields(args as unknown, ['operationId', 'bytes', 'signal'], maximum), maximum}, {fields: {bytes: {bytes: maximum}}});
    case 'payment_attempt_begin':
      return result({...fields(args as unknown, ['operationId', 'stepIndex', 'sourceId', 'routeBinding', 'mode', 'origin', 'wallTimeMs',
        'monotonicElapsedMs', 'observationSequence', 'policy', 'signal']), maximum: Math.min(pcztMaximum, 2 * 1024 * 1024)});
    case 'signer_authorize': {
      const input = copyRecord(args as unknown, ['token', 'format', 'parameters', 'genesis', 'height', 'branch', 'bytes', 'maximum', 'signal']);
      if (typeof input.maximum !== 'number' || !Number.isSafeInteger(input.maximum) || input.maximum < 1) throw invalidArgument();
