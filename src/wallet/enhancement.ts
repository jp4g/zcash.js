...
function sourcePoint(value: unknown) {
  ...
  if (typeof point.height !== 'number' || !Number.isInteger(point.height) || point.height < 0
    ...
    || typeof point.sourceId !== 'string' || !point.sourceId.length || point.sourceId.length > 256)
    throw protocol();
  try { return {height: point.height, hash: blockHash(point.hash), sourceId: point.sourceId}; }
...
function unspentItem(value: unknown, address: string, tipHeight: number) {
  ...
  let id: TxId;
  try { id = txId(item.txid); } catch { throw protocol(); }
  if (item.address !== address || typeof item.outputIndex !== 'number' || !Number.isInteger(item.outputIndex)
    || item.outputIndex < 0 || item.outputIndex > 0xffffffff || typeof item.value !== 'bigint' || item.value < 0n)
    throw protocol();
...
function unspentInventory(value: unknown, before: SourcePoint, address: string, start: number) {
  ...
  const seen = new Set<string>(), groups = new Map<TxId, UnspentGroup>();
  let scripts = 0;
...
function unspentTransaction(value: unknown, id: TxId, sourceId: string, height: number | null, maximum: number) {
  ...
  if (transaction.txid !== id || transaction.sourceId !== sourceId || observed.txid !== id || observed.sourceId !== sourceId)
    throw protocol();
  if (observed.state === 'mined') {
    const inclusion = evidence(observed.inclusion, ['height', 'blockHash', 'confirmations']);
    if (typeof inclusion.height !== 'number' || !Number.isInteger(inclusion.height) || inclusion.height !== height)
      throw protocol();
...
async function unspent(session: Session, light: LightClient, revision: string,
  ...
  try {
    ...
    if (local.revision !== revision || block.revision !== revision || local.tipHeight !== before.height
      || block.point?.height !== before.height || block.point.hash !== nativeHash)
      throw failure('RECOVERY_REQUIRED', 'sync', 'sync', 'Source tip is not the retained native chain point.');
    ...
    for (const [id, group] of groups) {
      ...
      const raw = unspentTransaction(supplied, id, before.sourceId, group.height, 2 * 1024 * 1024 - bytes - scriptBytes);
    ...
    if (after.height !== before.height || after.hash !== before.hash || after.sourceId !== before.sourceId) throw protocol();
    pending.check();
    await session.enhancement.apply({revision, request, result: {
