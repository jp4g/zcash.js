...
export function walletExecute(
  wallet: Awaited<ReturnType<typeof openWalletRuntime>>,
  accounts: ReturnType<typeof walletAccounts>,
  ...
  async function execute(
    ...
    const input = snapshot(args, kind === 'send'
      ? ['proposal', 'accountId', 'to', 'amount', 'memo', 'payments', 'idempotencyKey', 'maxFee', 'signer', 'signal']
      : ['accountId', 'fromAddresses', 'toPool', 'threshold', 'idempotencyKey', 'maxFee', 'signer', 'signal']);
    const { signer, signal, ...intent } = input,
    ...
    try {
      ...
      if ('proposal' in intent) {
        ...
        proposalBinding(intent.proposal, wallet.session);
        proposal = intent.proposal;
      } else {
        proposal = await propose(
          {
            ...intent,
            ...(kind === 'shield' ? { kind: 'shield' as const } : {}),
