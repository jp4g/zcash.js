...
function snapshot(args: object, maximum: number, command: WalletCommand, pcztMaximum: number) {
  ...
  const copy = (value: unknown, depth: number, bounds: CopyBounds = {}): unknown => {
    ...
    if (ArrayBuffer.isView(value)) {
      const owned = ownBytes(value, invalidArgument, limitError, Math.min(bounds.bytes ?? 4 * 1024 * 1024, Math.floor((maximum - size) / 2)));
      copied.push(owned);
...
export function attachWalletWorker(port: MessagePort, destroy: () => Promise<void>,
  ...
  const call = <C extends WalletCommand>(command: C, args: WalletInput<C>, close = false, cleanup = false): Promise<WalletResult<C>> => {
    ...
    return new Promise<WalletResult<C>>((resolve, rejectPromise) => {
      const job: Job = { id: ++nextId, command, args: admitted.args, size: admitted.size, ready: false, cancelled: false,
        cleanup: () => {}, resolve: value => resolve(value as WalletResult<C>), reject: rejectPromise };
  ...
  return {
    ...
    pczt: {
      finalize:(args:{operationId:string;artifactId:string;spend:Uint8Array;output:Uint8Array}&Op)=>call('pczt_finalize',args),
      ...
      prove: (args: {operationId:string;artifactId:string;spend:Uint8Array;output:Uint8Array} & Op) => call('pczt_prove',args),
    ...
    crashed,
    close(): Promise<void> {
      closing ??= call('close', {}, true).then(async () => { stopped = true; await dispose(); }, async error => {
        stopped = true;
