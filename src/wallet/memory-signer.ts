...
export function memorySignerAuthority(signer: Signer) {
  return authorities.get(signer);
}
...
const mismatch = () => failure(
  ...
  'authorization',
  'reattach-signer',
  'Signer cannot satisfy the request.',
);
...
export async function memorySigner(network: Network, authority: Authority): Promise<MemorySigner> {
  ...
  const check = () => {
    if (disposing) throw failure('CLOSED', 'authorization', 'none', 'Signer is disposed.');
    authority.check();
  };
  const sameNetwork = (value: Network) => {
    if (networkBinding(value).definition.binding !== bound.definition.binding) {
      throw failure(
        'NETWORK_MISMATCH',
        'authorization',
        'correct-input',
        'Signer network does not match.',
      );
    }
  };
  const signer = Object.freeze({
    async getCapabilities(args: Op = {}) {
      ...
      try {
        pending.check();
        check();
        return signerCapabilities(capabilities);
      } finally {
        pending.close();
      }
    },
    async getAccount(args: Parameters<Signer['getAccount']>[0]) {
      ...
      if (selector.kind !== 'derived'
        || accountIndex(selector.accountIndex) !== description.accountIndex) throw mismatch();
      const pending = operation(input.signal);
      let account: AccountDescriptor | undefined;
      try {
        pending.check();
        check();
        account = await accountFromViewingKey({
          network: input.network,
          format: 'ufvk',
          encoded: description.viewingKey,
        ...
        return Object.freeze({
          ...account,
          provenance: { accountIndex: accountIndex(description.accountIndex), scheme: 'zip32-and-bip44' as const },
        });
      } catch (error) {
        if (account) await account.viewing.dispose();
        throw error;
      } finally {
        pending.close();
      }
    ...
    async authorize(args: Parameters<Signer['authorize']>[0]) {
      ...
      try {
        ...
        pending.check();
        check();
        const bytes = await authority.authorize({
          format: bound.definition.parametersFormat,
          parameters: bound.definition.parameters.bytes,
          genesis,
          height: request.context.targetHeight,
    ...
    dispose() {
      return disposing ??= authority.dispose();
    },
  }) as MemorySigner;
