...
type Builtins = { 'fs/promises': typeof import('node:fs/promises'); os: typeof import('node:os'); path: typeof import('node:path') };
const builtin = <K extends keyof Builtins>(name: K) =>
  (globalThis as { process?: { getBuiltinModule?<T extends keyof Builtins>(name: T): Builtins[T] } }).process?.getBuiltinModule?.(name);
...
export class ProvingAssets {
  ...
  private async read(key:string,maximum:number):Promise<Uint8Array|undefined>{
    ...
    if(fs){
      ...
      let file;try{file=await fs.open(filename,'r');}catch(error){if(typeof error === 'object' && error !== null && 'code' in error && error.code === 'ENOENT')return;throw error;}
