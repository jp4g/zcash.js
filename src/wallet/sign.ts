...
export function canAuthorize(capabilities: SignerCapabilities, info: PcztInspection, profile: string): boolean {
  return capabilities.networks.includes(info.context.network.identity) && info.pools.every((pool) => {
    const circuit = pool === 'sapling'
      ? 'sapling-groth16/1'
      : pool === 'ironwood' ? 'ironwood-post-nu6_3/1' : undefined;
    ...
    try {
      ...
      if (proposal.steps.length !== 1) {
        throw failure(
          ...
          'authorization',
          'correct-input',
          'PCZT signing requires one step.',
        );
      }
      const accountId = proposal.accountIds[0];
      ...
      const signer = boundedSigner(authority, session.pczt.maximum),
        capabilities = await signer.getCapabilities({ signal: pending.signal });
      ...
      if (!retained || retained.accountId !== accountId
        || retained.artifactId !== binding.artifactId
        || retained.operationId !== binding.operationId) throw mismatch();
      const handle = await pczt.parse({
        bytes: retained.bytes,
        context: proposal.context,
        maxBytes: session.pczt.maximum,
      ...
      try {
        ...
        if (native) {
          ...
          if (await session.accounts.checkKey({
            accountId,
            viewingKey: description.viewingKey,
            signal: pending.signal,
          }) !== 'ready') {
            throw mismatch();
          }
        ...
        const bytes = native
          ? retained.bytes
          : (await proposals.export({ pczt: input.pczt, signal: pending.signal })).bytes;
        ...
        if (bytes.length > maximum) {
          throw failure(
            ...
            'authorization',
            'configure',
            'Signer PCZT exceeds limit.',
          );
        }
        pending.check();
        session.check();
        const result = await boundedSigner(signer, maximum).authorize({
          requestId: binding.artifactId,
          pczt: bytes,
          context: proposal.context,
          accountIds: proposal.accountIds,
          capabilityRevision: capabilities.revision,
          reviewCommitment: proposal.reviewCommitment,
          signal: pending.signal,
        });
        pending.check();
        session.check();
