...
export async function signerSelector(
  session: ReturnType<typeof attachWalletWorker>,
  account: AccountRecord,
  signal?: AbortSignal,
): Promise<SignerSelector> {
  ...
  if (key === null) {
    throw failure(
      ...
      'account',
      'reattach-signer',
      'Account has no full viewing key for signer lookup.',
    );
  }
