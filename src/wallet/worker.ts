...
function errorInfo(error: unknown, command: WalletCommand): { error: ErrorInfo; invalid: boolean } {
  ...
  const stage: ErrorInfo['stage'] = invalid
    ? 'runtime'
    : storage
      ? 'storage'
      : code === 'INVALID_ARGUMENT'
        ? 'validation'
        : definition.stage;
  const recovery: ErrorInfo['recovery'] = code === 'RESOURCE_LIMIT'
    ? 'configure'
    : invalid || storage
      ? 'reopen'
      : code === 'SYNC_REQUIRED' || sync && ['CURSOR_STALE', 'PROTOCOL_MISMATCH'].includes(code)
        ? 'sync'
...
export function installWalletWorker(
  owner: InitializedViews | undefined,
  port: MessagePort,
  ownerInvalid: () => boolean = () => false,
    ...
    try {
      ...
      const completion = session
        ? typeof error === 'object' && error !== null
          ? session.completion(error) ?? 'unknown'
          : 'unknown'
        : info.invalid ? 'unknown' : 'none';
