...
import { serveTransparentFixtures as serveFixtures, acceptedArtifacts, base64, frame, service } from './light-transparent-reads-fixtures.mjs';
import { transparentChecks } from './light-transparent-reads-checks.mjs';

export async function browserChecks() {
  const internal = await import('/src/clients/light-transparent-reads.js');
  ...
  const { createTransparentAddressCodec } = await import('/address/codec.mjs');
  const addressBytes = new Uint8Array(await (await fetch('/address/wasm/zakura_transparent_address_bg.wasm')).arrayBuffer());
  const result = await transparentChecks(createTransparentAddressCodec(addressBytes), createLightwire(bytes), internal, createGrpcWebByteTransport, location.origin);
...
async function runFirefox() {
  ...
  const logs = process.env.LIGHT_TRANSPARENT_LOGS ?? '/home/jack/zcash-light-transparent-reads-logs';
  const scratch = process.env.LIGHT_TRANSPARENT_SCRATCH ?? '/home/jack/zcash-light-transparent-reads-scratch';
  ...
  try {
    ...
    const build = process.env.LIGHT_TRANSPARENT_BUILD; assert.ok(build, 'LIGHT_TRANSPARENT_BUILD required');
    const assets = new Map([
      ['/', `<!doctype html><meta charset="utf-8"><title>gRPC-Web fixture</title><link rel="icon" href="data:,">
<script type="module">
import { browserChecks } from '/light-transparent-reads-browser.mjs';
try { window.grpcWebResult = await browserChecks(); }
      ...
      ['/light-transparent-reads-browser.mjs', readFileSync(new URL('./light-transparent-reads-browser.mjs', import.meta.url))],
    ...
    for (const request of fixture.requests) assert.ok(!request.headers.cookie && !request.headers.referer);
    for (const path of ['/address/codec.mjs','/address/wasm/zakura_transparent_address_bg.wasm','/codec/wasm/zakura_lightwire_bg.wasm']) assert.ok(fixture.served.includes(path));
    trace('checks-passed');
