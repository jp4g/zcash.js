...
const { decodeTransaction } = await import(pathToFileURL(`${root}/transaction.mjs`));
const beforeInit = admission(decodeTransaction, initialize, consensusContext, wasm);
initialize(wasm);
const transactionResult = transactions(decodeTransaction, JSON.parse(await readFile(`${root}/transaction-vectors.json`)));
...
console.log(JSON.stringify({ ...result, transactions: transactionResult, beforeInit, node: process.version, buffer: true, imports }));
