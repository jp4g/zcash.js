...
const pinBytes = readFileSync(join(here, 'pin.json')), pin = JSON.parse(pinBytes);
...
for (const name of ['src/runtime/primitive-loader.ts', 'src/runtime/artifacts.ts', 'src/errors.ts', 'docs/api/public-api.ts']) source.set(name, readFileSync(join(sdk, name)));
const temp = mkdtempSync(join(scratch, 'compile-'));
...
for (const name of ['browser.mjs', 'network-cases.mjs', 'transaction-cases.mjs', 'vectors.mjs', 'host.mjs']) files.set(name, readFileSync(join(here, name)));
const vectors = readFileSync(join(sdk, 'qualification/transaction-codec/fixtures/vectors.json'));
...
files.set('provenance.json', Buffer.from(JSON.stringify({ sdkBase: pin.sdkBase, sdkSourceState: 'uncommitted assigned changes; Git mount denied index.lock',
