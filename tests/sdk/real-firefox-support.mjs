...
export function verifyResult(result) {
  ...
  assert.deepEqual(result.pczts,Array.from({length:2},()=>({methods:5,v5:true,v6:true,structuralOnly:true,cancelled:2})));
  assert.deepEqual(result.signers,Array.from({length:2},()=>({methods:3,ownedBytes:true,actualViewing:true,cancelled:2,verifiedAuthorization:false})));
  assert.deepEqual(result.birthdays,Array.from({length:2},()=>({nativeValidated:true,nonpalindromicGenesis:true,cancelled:2})));
