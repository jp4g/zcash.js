...
import { accountFromViewingKey, viewing } from '../../dist/src/viewing.js';
import { fixture as viewingFixture } from '../sdk/viewing-fixture.mjs';
...
async function fixture({ finalized = true, steps = 1 } = {}) {
  const operationId = '01'.repeat(32),
    calls = [];
  const session = {
    check() {},
    completion() {},
    proposals: {
      async create() {
        return {
          ...
          reviewCommitment: '03'.repeat(32),
          accountId: 'account',
          revision: 'epoch:1',
          targetHeight: 101,
          branchId: bound.codec.consensusContext(
  ...
  const proposals = new WalletProposals(session, network),
    proposal = await proposals.create({});
  const state = {
    operationId,
    steps: Array.from({ length: steps }, () => ({ txid: finalized ? '04'.repeat(32) : null })),
  };
  const payments = {
    operations: {
      async get() {
  ...
  const api = walletExecute(
    { session },
    {
      attachedSigner() {
        throw Error('finalized retry must not acquire signer');
      },
    },
    proposals,
    payments,
...
test('finalized supplied plan and idempotent intent dispatch without signer, build or assets', async () => {
  ...
  assert.equal(calls[0].origin, 'send');
  await api.shield({ accountId: 'account', idempotencyKey: 'existing' });
  assert.equal(calls[1].kind, 'shield');
  assert.equal(calls[2].origin, 'shield');
});
test('custom multi-step authority is rejected before any signer method, retaining operation ID', async () => {
  ...
  const signer = new Proxy({}, {
    get() {
      throw Error('must not disclose to custom signer');
    },
  });
  await assert.rejects(
    api.send({ proposal, signer }),
    error => error.code === 'PCZT_MULTI_STEP_UNSUPPORTED' && error.operationId === proposal.operationId,
  );
...
test('mixed supplied-plan intent and cancellation cannot dispatch; partial bytes cannot rebuild', async () => {
  ...
  await assert.rejects(
    api.send({ proposal }),
    error => error.code === 'RECOVERY_REQUIRED' && error.operationId === proposal.operationId,
  );
  assert.equal(calls.length, 0);
});
test('custom role negotiation admits unproven signing before proving, without retrying a signer', async () => {
  const network = await defineNetwork({
    identity: 'role-order',
    genesisHash: pcztFixture.genesis,
    parametersFormat: 'zcash-js-network/1',
    parameters: new TextEncoder().encode(pcztFixture.parameters),
  });
  for (const proofState of ['either', 'required']) {
    ...
    const session = {
      ...
      proposals: {
        ...
        async create() {
          return {
            ...
            accountId: 'account',
            revision: 'epoch:1',
            targetHeight: pcztFixture.ironwoodHeight,
            branchId: pcztFixture.ironwoodBranch,
            lockExpiryHeight: 121,
            totalFee: 10000n,
            steps: [
              {
      ...
      pczt: {
        ...
        async get() {
          return {
            operationId,
            artifactId: '06'.repeat(32),
            accountId: 'account',
            outputs: [],
            bytes: Uint8Array.from(Buffer.from(pcztFixture.ironwood, 'hex')),
    ...
    const proposals = new WalletProposals(session, network),
      proposal = await proposals.create({});
    proposals.prove = async () => {
      calls.push('prove');
      throw failure(
        'ROLE_PRECONDITION',
        'proving',
        'correct-input',
        'Missing signer-owned material.',
      );
    };
    const signer = {
      async getCapabilities() {
        return {
          ...
          networks: ['role-order'],
          maxPcztBytes: 65536,
          exportableViewing: ['ufvk'],
          accountDiscovery: 'explicit-index',
          authorizations: [
            {
              pool: 'ironwood',
              txVersion: 6,
              branchIds: [pcztFixture.ironwoodBranch],
              pcztVersions: [2],
              circuitVersions: ['ironwood-post-nu6_3/1'],
              proofState,
              requiredFields: ['zakura-signer-full/1'],
              review: 'device',
            },
          ],
        };
      ...
      async getAccount() {
        ...
        throw failure(
          ...
          'authorization',
          'none',
          'Fixture stops before disclosure.',
        );
      },
      async authorize() {
    ...
    const payments = {
      operations: {
        async get() {
          return { operationId, steps: [{ txid: null }] };
        },
      },
      async dispatch() {
    ...
    const api = walletExecute({ session }, {
      attachedSigner() {
        return signer;
      },
    }, proposals, payments, async () => proposal);
    await assert.rejects(
      api.send({ proposal }),
      { code: proofState === 'either' ? 'SIGNER_REJECTED' : 'ROLE_PRECONDITION' },
    );
    assert.deepEqual(calls, [proofState === 'either' ? 'signer' : 'prove']);
    if (proofState === 'either') {
      const descriptor = await accountFromViewingKey({
        network,
        format: 'ufvk',
        encoded: viewingFixture.ufvk,
        enabledPools: ['transparent', 'sapling', 'ironwood'],
      });
      try {
        calls.length = 0;
        signer.getAccount = async () => {
          calls.push('account');
          return descriptor;
        };
        session.accounts.checkKey = async () => {
          calls.push('match');
          return 'recovery-required';
        };
        await assert.rejects(api.send({ proposal }), { code: 'SIGNER_CAPABILITY_MISMATCH' });
        assert.deepEqual(calls, ['account', 'match']);
        await assert.rejects(viewing.export({
          account: descriptor,
          format: 'ufvk',
