...
if (typeof process !== 'undefined' && process.versions?.node) {
  ...
  try {
    ...
    if(process.env.WALLET_LOADER){assert.equal(answer.value.publicSync,true);assert.equal(answer.value.emptyCompleted,true);assert.equal(answer.value.watchShared,true);assert.equal(answer.value.enhancementPending,true);assert.equal(answer.value.rewoundTo,99);assert.equal(answer.value.enhanced,true);}
    assert.equal(answer.value.workerDestructions, process.env.WALLET_LOADER ? 8 : 2);
